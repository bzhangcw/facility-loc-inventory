...
def check_inventory_n2(model, j):
    ...
    if j.end.type == const.WAREHOUSE:
        ...
        if str(j.start).startswith("F"):
            ...
            if int(t) != -1:
                last_period_inventory = model.variables["sku_inventory"].sum(
                    t, j.end, "*"
                )
...
def seperation_gcf(model, x, y, t, N1, N2, d, dump=False, verbose=False):
    ...
    md.addConstr(
        sum(
            alph[idx] * j.capacity + beta[idx] * j.variable_lb
            for idx, j in enumerate(N1)
        )
        - sum(
            delt[idx] * j.capacity + gamm[idx] * j.variable_lb
    ...
    for idx, j in enumerate(N2):
        ...
        e2 += (temp_x + j.variable_lb * (1 - temp_y)) * gamm[idx] + j.capacity * delt[
            idx
        ]
    ...
    for idx, j in enumerate(N2):
        ...
        md.addConstr(
            v1[idx]
            == (temp_x - (lp2[idx] - lbd) * temp_y) * (1 - gamm[idx] - delt[idx])
        )
        md.addConstr(v2[idx] == mu2[idx] * temp_y * (1 - gamm[idx] - delt[idx]))
        md.addConstr(v3[idx] == temp_x * (1 - gamm[idx] - delt[idx]))
        md.addConstr(v4[idx] == grb.min_([v1[idx], v2[idx], v3[idx]]))
