...
def read_data(  data_dir,
    ...
    plant_num = min(plant_num, len(node_df.query("id.str.startswith('P')",engine="python")))
    warehouse_num = min(warehouse_num, len(node_df.query("id.str.startswith('T')",engine="python")))
    customer_num = min(customer_num, len(node_df.query("id.str.startswith('C')",engine="python")))
    ...
    node_df = pd.concat([node_df.query("id.str.startswith('P')",engine="python").iloc[:plant_num],\
                        node_df.query("id.str.startswith('T')",engine="python").iloc[:warehouse_num],\
    ...
    # ==================== construct plant ============================
    plant_df = node_df.query("id.str.startswith('P')",engine="python")[['id', 'total_capacity']]
    plant_sku_df_dict = dict(list(node_sku_df.query("id.str.startswith('P')",engine="python").groupby('id')))
    ...
    # ==================== construct warehouse ============================
    warehouse_df = node_df.query("id.str.startswith('T')",engine="python")[['id', 'total_capacity', 'fixed_cost', 'if_current']]
    warehouse_sku_df_dict = dict(list(node_sku_df.query("id.str.startswith('T')",engine="python").groupby('id')))
    warehouse_sku_time_df_dict = dict(list(node_sku_time_df.query("id.str.startswith('T')",engine="python").groupby('id')))
    ...
    customer_sku_time_df_list = dict(list(node_sku_time_df.query("id.str.startswith('C')",engine="python").groupby('id')))
    ...
    for edge in edge_sku_df_list:
        ...
        unit_cost = edge_df[['sku', 'unit_cost']]
        capacity = np.inf
