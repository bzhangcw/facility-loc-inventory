...
def set_state(self):
    if DEFAULT_ALG_PARAMS.phase2_use_full_model == 1:
        ...
        x_w_rhs = {
            (*k, t): 0 if k not in set(self.data.warehouse_routes) else 1e4
            for k in self.data.X_W
            for t in self.data.T
        }
        x_c_rhs = {
            (*k, t): 0 if k not in set(self.data.available_routes) else 1e4
            for k in self.data.X_C
            for t in self.data.T
        }

        if self.state_constrs.__len__() == 0:
            bound_constr_w = model.addConstrs(
                (x_w[k] <= x_w_rhs[k] for k in self.data.X_W)
            )
            bound_constr_c = model.addConstrs(
                (x_c[k] <= x_c_rhs[k] for k in self.data.X_C)
            )
