...
if __name__ == "__main__":
    ...
    # arg.add_inv = 1
    # if arg.add_inv == 1:
    #     datapath = "/Users/xue/github/facility-loc-inventory/data/data_0401_1inv.xlsx"
    # else:
    #     datapath = "/Users/xue/github/facility-loc-inventory/data/data_0401_0inv.xlsx"
    pick_instance = 1
    if pick_instance == 1:
        ...
        while k < 20:
            ...
            for e in utils.get_out_edges(network, end):
                if e.capacity > 1e6:
                    ...
                    e.capacity = 0.4e5
            n1_temp = Warehouse(
                idx="Feak{}T{}".format(end, t),
                location=[],
                inventory_capacity=end.inventory_capacity,
                inventory_lb=0,
            )
            n2_temp = Warehouse(
                idx="Feak{}T{}".format(end, t - 1),
                location=[],
                inventory_capacity=end.inventory_capacity,
                inventory_lb=0,
            )
            ...
            cutv, lbdv, subsets = seperation_gcf(
                model, x, y, t, N1, N2, d, dump=False, verbose=False
            )
            print("lbd", lbdv)
            if cutv > 1e-2:
                # 修改点2
                print("---------Add Cut----------")
                expr, cut_value, lbd, bool_voilate = eval_cut_c3(
                    model, x, y, t, d, *subsets, lbdv=lbdv
                )
                ...
                print(
                    ...
                    mipval,
                    model.model.objval - mipval / mipval,
                )
                # print(df_edges['y'])
                for _, row in df_edges.iterrows():
                    if 0 < row["y"] < 1:
            ...
            k += 1
        print(
            "After adding cuts for {}th fractional inter edge, then we have:".format(n)
        )
        for _, row in df_edges.iterrows():
            if 0 < row["y"] < 1:
                print(row["obj_start"], row["obj_end"], row["t"], row["y"])
