...
def generate_instance(
    ...
    customer_sku_demand_df = customer_sku_demand_df[customer_sku_demand_df["id"].isin(customer_df["id"]) & customer_sku_demand_df["sku"].isin(sku_df["id"])]
    ...
    for w in warehouse_df.index:
        ...
        temp_df_w = pd.DataFrame.from_dict(temp_w, orient='index', columns=['Unit_cost']).rename_axis('sku').reset_index()
            ...
            if warehouse_num > 1000:
                ...
                for warehouse in warehouse_list:
                    for sku in sku_list:
                        ...
                        edges_w_c.append({'start_id': warehouse.idx, 'end_id': customer.idx, 'sku': sku.idx, 'unit_cost': unit_cost})
            ...
            if warehouse_num > 1000:
                ...
                for warehouse in warehouse_list:
                    for sku in sku_list:
                        ...
                        edges_p_w.append({'start_id': plant.idx, 'end_id': warehouse.idx, 'sku': sku.idx, 'unit_cost': unit_cost})
    ...
    edge_sku_df = edge_sku_df[
        edge_sku_df["start_id"].isin(node_df["id"]) & edge_sku_df["end_id"].isin(node_df["id"]) & edge_sku_df["sku"].isin(sku_df["id"])
    ]  
