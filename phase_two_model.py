...
class PhaseTwo:
    ...
    def build(self):
        ...
        inv_avail = model.addVars(self.data.I, self.data.S, self.data.T_t, nameprefix="inva",
                                  vtype=COPT.CONTINUOUS)  # 可用库存量
        surplus_inv = model.addVars(self.data.I, self.data.S, self.data.T_t, nameprefix="invs", vtype=COPT.CONTINUOUS)
        ...
        self.variables = z_p, z_l, x_p, x_w, x_c, inv, surplus_inv, inv_gap, inv_f, inv_avail, storage_gap, end_inv_gap
        ...
        self.obj_expr_map = dict(
            ...
            ) for i, j, s, t in x_w if (i, j) not in self.data.weird_pairs),
            cost_w2w_weird=5000 * quicksum(x_w[i, j, s, t] * (
                    self.data.added_warehouse_cost.get(i, 0) / 1e3 + self.data.warehouse_transfer_cost[i, j, s]
            ) for i, j, s, t in x_w if (i, j) in self.data.weird_pairs),
